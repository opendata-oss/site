...
Today we're announcing OpenData *Buffer* to make that additional system braindead simple and cheap.
...
The details are in the [OpenData Buffer README](https://github.com/opendata-oss/opendata/tree/main/ingest), the [Buffer design RFC](https://github.com/opendata-oss/opendata/blob/main/ingest/rfcs/0001-stateless-ingest.md), and the [Timeseries ingest-consumer RFC](https://github.com/opendata-oss/opendata/blob/main/timeseries/rfcs/0006-ingest-consumer.md).
...
In practice "slower" still means *p99 latencies of under 1s*, which is acceptable for many high-throughput workloads (particularly in telemetry and observability). Furthermore, the latency/cost tradeoff is configurable. Increasing the frequency of flushes and polls will lower your latency (to a floor of approximately 50-100ms) at increased object storage request costs.
...
The next step is fleshing out the first party integrations. We already have an OpenTelemetry exporter that allows you to ship metrics from any OTel collector deployment to *Buffer* and ingest it into *Timeseries*. We are actively working on native destinations for ClickHouse and various other sinks.
...
*Buffer* is MIT-licensed and available today as part of [OpenData](https://github.com/opendata-oss/opendata/). You can read the [design RFC](https://github.com/opendata-oss/opendata/blob/main/ingest/rfcs/0001-stateless-ingest.md) and try it through the [OpenData Timeseries quickstart](https://www.opendata.dev/docs/timeseries/quickstart).
